"""
Script to remove methods from batch_processing_page.py that have been moved to ProcessingMixin.

Methods to remove:
- _start_processing
- _run_batch_processing
- _process_with_memory_management
- _process_with_turbo_mode
- _process_single_file_safe
- _handle_batch_cancelled
"""
...
def update_class_declaration(content: str) -> str:
    """Update the class declaration to inherit from ProcessingMixin."""
    old = 'class BatchProcessingPage(ExportMixin, ctk.CTkFrame):'
    new = 'class BatchProcessingPage(ProcessingMixin, ExportMixin, ctk.CTkFrame):'

    if old in content:
        print("  Updating class declaration to inherit from ProcessingMixin")
...
def update_imports(content: str) -> str:
    """Update the import statement to include ProcessingMixin."""
    old_import = 'from laser_trim_analyzer.gui.pages.batch.export_mixin import ExportMixin'
    new_import = '''from laser_trim_analyzer.gui.pages.batch.export_mixin import ExportMixin
from laser_trim_analyzer.gui.pages.batch.processing_mixin import ProcessingMixin'''

    if old_import in content and 'ProcessingMixin' not in content:
        print("  Updating imports to include ProcessingMixin")
...
def update_docstring(content: str) -> str:
    """Update the class docstring to mention ProcessingMixin."""
    old_docstring = '''"""Batch processing page with comprehensive validation and responsive design.

    Uses mixins for modular functionality:
    - ExportMixin: Excel, CSV, HTML export methods
    """'''

    new_docstring = '''"""Batch processing page with comprehensive validation and responsive design.
...
def main():
    ...
    file_path = project_root / 'src' / 'laser_trim_analyzer' / 'gui' / 'pages' / 'batch_processing_page.py'
