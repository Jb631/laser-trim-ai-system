...
class NonTrimWorkbookError(Exception):
    """A KNOWN non-data workbook (machine parameter/report sheets) that is
    named like trim data. Callers skip these like other non-trim files —
    distinct from ValueError, which stays reserved for truly UNKNOWN layouts
    (the loud third-laser guard)."""
...
class ExcelParser:
    ...
    def _detect_system_from_sheets(self, sheet_names: List[str]) -> SystemType:
        ...
        # Machine parameter/report workbooks ('Model Parameters', 'Trim
        # Parameters', 'Report', …) are sometimes named like test data
        # (e.g. '8601_69_TEST DATA_…') but contain NO sweep. They are a KNOWN
        # non-data layout — skip them like other non-trim files instead of
        # raising the might-be-the-third-laser error (found 2026-07-08 when
        # the parse-regression harness met the loud unknown-layout guard).
        lowered = {s.strip().lower() for s in sheet_names}
        if {"model parameters", "trim parameters"} <= lowered:
            raise NonTrimWorkbookError(
                f"parameter/report workbook, no test sweep (sheets: {sheet_names})")
    ...
    def _extract_track_data(
        ...
        try:
            ...
            # NOT the actual measured angle. Only read from System A trimmed sheets.
            measured_electrical_angle = None
            if "measured_electrical_angle" in cells and system_type != SystemType.B:
