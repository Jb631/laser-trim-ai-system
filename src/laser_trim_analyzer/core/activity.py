...
before the newest trim file in the whole database -- or when it has laser files but no trim at all
("no trims on record": never trimmed is certainly not trimmed in two years). It is LABELLED, never
hidden: every finding and every number stays where it was.
...
refreshed. Measured on the 6.2 GB work database (2026-09-25, read-only): about 67 ms warm; 326
models with laser files, 196 inactive -- 190 by date and 6 with no trim on record.
"""
...
# that is a TRIM -- NULL when none is, which is what "no trims on record" reads. "Has a track that
# is a trim" is asked as "has a track, and not every one of its tracks is not-a-trim": the same set,
# answered from the indexes alone. Asking each track's own status reads every track row, sweep
# arrays and all: 3.7 s cold, 122 ms warm, against 67 ms (tests/test_model_activity.py checks the
# two forms agree on files of every shape).
_SQL = """
SELECT a.model,
       MAX(CASE WHEN EXISTS (SELECT 1 FROM track_results t WHERE t.analysis_id = a.id)
                 AND a.id NOT IN (
                     SELECT f.analysis_id FROM track_results f
                     WHERE f.status IN ({not_a_trim})
                     GROUP BY f.analysis_id
                     HAVING COUNT(*) = (SELECT COUNT(*) FROM track_results c
                                        WHERE c.analysis_id = f.analysis_id))
                THEN a.file_date END)
...
class Activity:
    """One screen load's answer: every model's newest trim file, the fleet's, and the models with
    believable laser files but no trim among them."""
...
def load_activity(db, now: Optional[datetime] = None) -> Activity:
    ...
    with db.session() as s:
        rows = s.execute(text(_SQL.format(not_a_trim=not_a_trim)), params).fetchall()
    newest, never = {}, set()
