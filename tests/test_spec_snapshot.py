...
def _invented_ml():
    ...
    # A forest with n_jobs=-1 sums its trees across threads in a varying order: the last bit of
    # a probability varies call to call (2026-09-25, measured on the real predictors). The
    # trainer now leaves it on one thread itself (Task 11); explicit here too, so the exact
    # comparisons below never depend on that.
