...
def classify(ftype: str, relpath: str) -> Dict[str, Any]:
    ...
    from laser_trim_analyzer.core.parser import NonTrimWorkbookError
    ...
    try:
        if ftype == "trim":
            try:
                ...
                # Known non-data workbook -> skipped by the processor; ranks
                # as 'empty' (it never produced usable data on any version).
                return {"status": "empty", "reason": f"non-trim workbook: {e}"}
